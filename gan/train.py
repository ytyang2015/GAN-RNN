...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250,
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            my_noise = sample_noise(batch_size, noise_size).view(-1, G.noise_dim, 1, 1)
            if torch.cuda.is_available(): my_noise = my_noise.cuda()
            fake_images = G(my_noise).view(batch_size, input_channels, img_size, img_size)
            if torch.cuda.is_available(): fake_images = fake_images.cuda()
            G_solver.zero_grad()
            ...
            if torch.cuda.is_available(): fake_images = fake_images.cuda()
            D_solver.zero_grad()
            d_error = discriminator_loss(D(real_images).view(-1), D(fake_images).view(-1))
            d_error.backward()
            D_solver.step()
